...
def start_game():
    ...
    session['sector'] = {'A':0, 'B':0, 'C':0, 'D':0}
    session['sector'][sector] += 1
    session['score'] = 0
...
def check_riddle():
    ...
    if validate_riddle(riddle, answer):
        session['score'] += 1
        for sector, riddles in loc_rid.items():
            for curr_riddle, answer in riddles.items():
                if curr_riddle == riddle:
                    break
        if session['sector'][sector] == 2:
            question = loc_ques[sector][-1]
            render_template('question.html', question_text=list(question.keys())[0])
            session['sector'][sector] += 1
        if session['sector'][sector] == 3:
            sectors = list(session['sector'].keys()).pop(sector) #removving the sector that has been completed
